...
interface ElektrikOffersCatalogProps {
  formAnchor?: string;
}

export function ElektrikOffersCatalog({ formAnchor = "#lead-form" }: ElektrikOffersCatalogProps) {
  const groups = getCategoryOffersGrouped("elektrik");

  return (
    <Section id="service-catalog" className="bg-white">
      <SectionHeader
        badge="Услуги"
        title="Все услуги электрика"
          ...
          <div key={group.title}>
            ...
            <ul className="divide-y divide-slate-100 overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
              {group.offers.map((offer) => {
                const href = getServiceOfferPath("elektrik", offer.slug);
